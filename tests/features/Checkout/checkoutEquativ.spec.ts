...
/**
 * This test checks the Equativ checkout workflow
 * Equativ -> Direct Integration -> Client ID -> Secret Client ID -> Segment Provider ID
...
Object.values(roles).map((role) => {
  test.describe(`Checkout process for ${role.name}`, () => {
    ...
    test(`Happy Path Checkout Test Equativ, Direct Integration`, async ({
      page,
    }) => {
      const dsp = {
        name: "Equativ",
        product: {
          ...
          label: "Client ID",
          secret_key_label: "Client Secret",
          other_label: "Segment Provider ID",
        },
      };
      ...
      if (role.name === "Admin") {
        ...
        await page.getByLabel(`${dsp.product.other_label} *`).fill("123");
        ...
        await expect(page.getByText(dsp.product.other_label)).toBeVisible();
    ...
    test(`Checkout Test DSP Labels and Validity - Equativ, Direct Integration`, async ({
      page,
    }) => {
      const dsp = {
        name: "Equativ",
      ...
      if (role.name === "Admin") {
        ...
        await page.getByLabel(`${dsp.product.secret_key_label} *`).fill("123");
        await page.getByLabel(`${dsp.product.other_label} *`).fill("123");
        await expect(page.getByTestId("currency")).not.toBeVisible();
        await page.getByTestId("validate_dsp_on_lotame").click();
        await expect(
          page.getByTestId("datadesk-order-actions-top-next"),
        ).toBeEnabled();
...
//client id, client secret, segment provider, product platform or next button disabled
